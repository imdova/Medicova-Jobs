...
const EditCompanySection: React.FC<EmployerHeaderSectionProps> = ({ company, isEmployee }) => {
    ...
    const { data: sectorsData } = useFetch<PaginatedResponse<Sector>>(API_GET_COMPANY_SECTORS, {}, (sectors) => setSector(sectors.data?.find((sector) => sector.id === company?.companySectorId) || null));
    const { data: typesData } = useFetch<PaginatedResponse<Sector>>(sector ? API_GET_COMPANY_TYPES_BY_SECTOR + sector.id : null, {
        fetchOnce: false,
    ...
    const handleUpdate = async (formData: Partial<Company>) => {
        const { name, title, city, companySectorId, companyTypeId, state: formState, country: formCountry, } = formData
        const sector = sectors.find((sector) => sector.id === companySectorId) || null
        const type = types.find((type) => type.id === companyTypeId) || null
        const country = countries.data.find((country) => country.isoCode === formCountry?.code) || null
        const state = states.data.find((state) => state.isoCode === formState?.code) || null
        const body = {
            id: company?.id,
    ...
    const close = () => { setIsModalOpen(false); reset(); setCountryCode(company?.country?.code || ""); setSector(sectors?.find((sector) => sector.id === company?.companySectorId) || null) };
    const fields: FieldConfig[] = [
        ...
        },
        {
            ...
            required: true,
            onChange: (value) => setSector(sectors?.find((sector) => sector.id === value) || null),
        ...
        },
        {
            ...
            options: countries.data.map((country) => ({ value: country.isoCode, label: country.name })),
        ...
        },
        {
            ...
            options: states.data.map((state) => ({ value: state.isoCode, label: state.name })),
            gridProps: { xs: 6, md: 4 },
        },
        {
            name: "city",
    ...
    return (
        <div className="col-span-12 sm:col-span-3">
            ...
            />}
            <div className="flex flex-col gap-2 justify-center items-center sm:items-end h-full">
                {/* Edit Button */}
                {isEmployee && (
