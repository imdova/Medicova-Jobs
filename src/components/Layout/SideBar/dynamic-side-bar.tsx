...
export default function DynamicSideBar({ user }: { user?: User }) {
  ...
  return isFull || isMinimal ? (
    <aside
      className={`${isFull ? "w-[50px] min-w-[50px] xl:w-[250px]" : "w-[50px] min-w-[50px]"} group/sideBard ml-5 hidden transition-all duration-300 md:block`}
    >
      <div
      ...
      {/* <div
        aria-hidden="true"
        className={`${isMinimal ? "opacity-0 group-hover/sideBard:opacity-100" : "opacity-0"} pointer-events-none fixed inset-0 z-[29] h-dvh w-screen bg-black/30 backdrop-blur-sm duration-300`}
      ></div> */}
    </aside>
  ) : null;
}
